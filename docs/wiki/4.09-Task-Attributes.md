...
- **Timeline** — Internal stages and nodes for planning long-horizon work on top-level tasks.
...
A top-level task can have an internal timeline for long-horizon work. Timelines are separate from subtasks: they do not create task entities, do not change the task estimate, and do not appear in project or tag task lists.
...
Use the Timeline panel in task details to add stages and nodes, then start, complete, reopen, or delete nodes as the work progresses.
